...
public class HttpEnginer {
    ...
    public <T> Observable<Optional<T>> post(IConverter<T> converter) {
        RequestParamMap paramMap = onPreRequest(mHeaders, mParams);
        return mHttpEnginerConfig.getHttpRequest().post(mHttpEnginerConfig, mUrl, paramMap.getHeaders(), paramMap.getParams(), converter);
    }

    //异步请求，后面的流操作直接在主线程中执行
    public <T> Observable<Optional<T>> postAsync(IConverter<T> converter) {
        RequestParamMap paramMap = onPreRequest(mHeaders, mParams);
        return mHttpEnginerConfig.getHttpRequest().postAsync(mHttpEnginerConfig, mUrl, paramMap.getHeaders(), paramMap.getParams(), converter);
    }
